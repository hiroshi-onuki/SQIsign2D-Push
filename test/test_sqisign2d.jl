...
function check(param::Module, num::Int, is_compact::Bool)
    ...
    for i in 1:num
        pk, (s0, s1, M0, M1, M, xPm, xQm, xPQm, xP, xQ, xPQ, I) = param.key_gen(global_data)
        param.PushRandIsog(BigInt(13123214321), s0, s1, xPm, xQm, xPQm, xP, xQ, xPQ, M0, M1, global_data)
        println(i)
        #=
        m = "hello"
